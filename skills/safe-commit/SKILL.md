...
version: 1.0.0
user-invocable: true
allowed-tools: Bash(${CLAUDE_SKILL_DIR}/scripts/get_context.sh:*), Bash(git commit *), Bash(git -C * commit *), Bash(git diff *), Bash(git -C * diff *)
---
...
3. If the context output contains an `unstaged or untracked:` section, abort with an error listing those lines and tell the user to stage all intended changes before running `/safe-commit`.

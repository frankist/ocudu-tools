...
3. If the context output contains an `unstaged or untracked:` section:
   - **Unstaged changes** (lines starting with `M`, `A`, `D`, etc.) — automatically stage them with `git add` before continuing.

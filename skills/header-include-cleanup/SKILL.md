...
directory, ask for one; don't infer scope from context. When the ask is about build cost
rather than a named directory ("the build is too slow", "where is the header bloat"), run
Phase 0b first and propose a target from the ranking.
...
The cost ranking in Phase 0b additionally needs PyYAML:
```bash
...
## Phase 0b - Rank the headers by cost

Optional, but run it whenever the user's goal is "make the build cheaper" rather than
"tidy up this one directory" - it is what turns a guess about where the bloat is into a
list. Two steps, both writing to the build directory (outside the repo, so nothing here
...
counting each header **once per TU** - the include guard makes every reach after the first
free. That product is the point: a 12-line header in 4000 TUs and a 4000-line header in 12
TUs cost the build the same, and only one of them looks like a problem by eye. Pick the
...
- To spot-check a `tu_count`, compare it against what the compiler itself recorded:
  ...
  counts outputs the tree skips. A large gap on one header means its edges are going
  unresolved, not that it is cheap.

This is a **static estimate in physical lines**, for choosing targets. It is not a
measurement of what any edit saved: lines are not uniformly expensive and the estimate
cannot see the preprocessor. Once the sweep runs, "Measuring the win" below is the number
that settles whether it actually helped.
...
- Compare the two `mem-report.txt` files: the mean/median/max peak-RSS lines, and whether
  ...
  offenders at all is a sign the wrong target was picked - cross-check against Phase 0b's
  cost ranking, which is exactly what it exists to catch before the fact next time.
